...
checkCommitReadiness() {
  ...
  do
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID mychannel --name bigdatacc_$1 --signature-policy "OR('Org1MSP.peer', 'Org2MSP.peer')" --version 0 --init-required --sequence 1 >&log.txt
    res=$?
    set +x
